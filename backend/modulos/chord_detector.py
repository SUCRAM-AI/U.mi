...
def upload_audio(file_path_or_data: Union[str, bytes, BinaryIO], is_bytes: bool = False):
    """Envia o áudio e retorna a URL pública para usar no job.
    
    Args:
        file_path_or_data: Caminho do arquivo (str), dados binários (bytes) ou arquivo aberto (BinaryIO)
        is_bytes: Se True, trata file_path_or_data como bytes ao invés de caminho de arquivo
    """
    ...
    # 2️⃣ Faz o upload do arquivo ou dados
    if is_bytes or isinstance(file_path_or_data, bytes):
        # Se for bytes, fazer upload direto
        print(f"📤 Enviando dados de áudio (tamanho: {len(file_path_or_data) if isinstance(file_path_or_data, bytes) else 'N/A'})")
        put_resp = requests.put(data["uploadUrl"], data=file_path_or_data)
    elif hasattr(file_path_or_data, 'read'):
        # Se for um arquivo aberto (BinaryIO)
        print(f"📤 Enviando arquivo aberto")
        file_path_or_data.seek(0)  # Voltar ao início do arquivo
        put_resp = requests.put(data["uploadUrl"], data=file_path_or_data)
    else:
        # Se for caminho de arquivo (comportamento original)
        print(f"📤 Enviando arquivo: {file_path_or_data}")
...
def get_chords_from_audio(audio_path_or_data: Union[str, bytes, BinaryIO], workflow_id="untitled-workflow-18c7355", is_bytes: bool = False):
    """Detecta acordes de um áudio.
    
    Args:
        audio_path_or_data: Caminho do arquivo (str), dados binários (bytes) ou arquivo aberto (BinaryIO)
        workflow_id: ID do workflow da API Music.ai
        is_bytes: Se True, trata audio_path_or_data como bytes ao invés de caminho de arquivo
    
    Returns:
        Lista de acordes detectados (ex: ['C', 'Am', 'F'])
    """
    audio_url = upload_audio(audio_path_or_data, is_bytes=is_bytes)
...
def detect_chord_from_bytes(audio_bytes: bytes, workflow_id="untitled-workflow-18c7355"):
    """Função auxiliar para detectar acorde a partir de bytes de áudio.
    ...
    Returns:
        String com o acorde detectado (ex: 'C', 'Am', 'F')
    """
    acordes = get_chords_from_audio(audio_bytes, workflow_id=workflow_id, is_bytes=True)
    # Retorna o primeiro acorde detectado, ou None se não houver
    return acordes[0] if acordes else None